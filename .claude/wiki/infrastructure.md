...
| Redis | 6379 | Docker `infra/docker/docker-compose.yml`'s `redis` service |
| Memcached | 11211 | Local Homebrew `memcached`, or Docker's `memcached` service (same compose file) |
...
If either Postgres or Memcached service isn't running, `brew services list` shows it, `brew services start <name>` fixes it — this has already come up more than once (see [mistakes.md](mistakes.md)).
...
| `REDIS_URL` | Docker's redis container, port 6379 |
| `MEMCACHED_SERVERS` | `host:port` format (not a URL) — memjs's server-list syntax |
